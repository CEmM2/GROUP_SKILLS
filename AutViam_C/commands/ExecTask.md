...
`<skill_root>/autviam_c_config.json` if present, test each skill's `trigger_patterns`
...
Dispatch `autviam-spec-reviewer` as a Codex `explorer` prompt profile (see ExecPhase Step 6 for the prompt template). Parse verdict. If Codex agent dispatch is unavailable, run the profile inline and record `review_mode="inline"`. On FAIL: record + increment + loop.
...
`<skill_root>/autviam_c_config.json` if present, test each specialist's `trigger_patterns`
...
Dispatch `autviam-domain-reviewer` as a Codex `explorer` prompt profile. On FAIL: record + increment, re-run **Gate A then Gate B**.
...
1. Shell: `gh issue view <phase_issue> --json body -q .body`
...
4. Shell: `gh issue edit <phase_issue> --body-file /tmp/phase_<N>_body.md`
