...
Read `<skill_root>/autviam_c_config.json` if present.
...
```
Build `specialist_agents`: include only specialists where at least one diff file matched. Each entry must include `name`, `codex_agent_type` (default `explorer`), `prompt_file`, `description`, and `trigger_patterns`.
...
1. Shell: `gh issue view <plan_overview_issue> --json body -q .body`
