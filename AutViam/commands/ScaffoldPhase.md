...
Run a single `Glob` for `tests/**/*.py` and take the first 60 results. Group by top-level subdirectory (e.g. `tests/unit/`, `tests/integration/`). Build a one-paragraph "test layout summary" string:
...
This single string is reused for every subagent in Step 4. Do **not** re-glob per task.
...
Write `<tasks_folder>/Phase_<phase_id>_Scaffold_Validation.md`:
