...
**Output is transient** — `~/.agent/diagrams/<slug>.html`, an understanding aid, not a planning asset. (For a durable, reusable module/feature snapshot, use `/AutViam arch` instead — it writes to `dev/architecture/` with a planning digest.)
...
- Resolve the target to its definition(s). For a symbol: read it plus its callers and callees (use the repo's code-intel — e.g. `gitnexus_context`/`gitnexus_query` here — or grep). For a flow: trace the execution path across files. For a concept: find the code that embodies it (the gate, the handshake, the registry).
...
| "what is the shape of this whole module" | → that's **`/AutViam arch`**, not explain | — |
