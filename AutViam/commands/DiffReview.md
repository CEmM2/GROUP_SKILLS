...
Red = removed/before, green = added/after, amber = modified, blue = neutral context. When reviewing a plan's phase or full run, write to that plan's `<reviews_folder>` (`dev/plans/<plan_file_stem>/reviews/`); for an ad-hoc diff with no plan, `dev/diagrams/`. Open it and tell the user the path.
